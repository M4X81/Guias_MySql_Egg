...
from pago
where (date_format(fecha_pago, '%Y') = '2008') and forma_pago = 'PayPal'
order by total desc;
...
select nombre,cantidad_en_stock,precio_venta,gama
from producto
...
select c.codigo_cliente ,c.nombre_cliente, e.nombre, p.id_transaccion
from cliente c
join empleado e on c.codigo_empleado_rep_ventas = e.codigo_empleado
...
*/
select c.nombre_cliente, e.nombre as representante_venta, o.ciudad
from cliente c
join empleado e on c.codigo_empleado_rep_ventas = e.codigo_empleado
left join pago p on p.codigo_cliente = c.codigo_cliente
join oficina o on e.codigo_oficina= o.codigo_oficina
...
-- o se puede hacer con subconsultas:
select  distinct (select nombre_cliente from cliente where codigo_cliente= p.codigo_cliente) as nombre_cliente, t1.gama
from (select distinct pr.gama, dp.codigo_pedido as codigo from producto pr join detalle_pedido dp on pr.codigo_producto = dp.codigo_producto ) as t1
join pedido p on t1.codigo =  p.codigo_pedido
