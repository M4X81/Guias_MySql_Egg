...
select nombre, max(sal_emp) as maximo from empleados
group by nombre
...
select max(sal_emp) as maximo, min(sal_emp) as minimo, (max(sal_emp) - min(sal_emp)) as diferencia
from empleados;

-- 24. Hallar el salario promedio por departamento
select id_depto, round(avg (sal_emp),2)as salario_promedio from empleados
group by id_depto;
-- ó agregando el join pero guarda menos resultados xq los unifica los departamentos

SELECT d.nombre_depto, ROUND(AVG(e.sal_emp), 2) as salario_promedio
FROM empleados e
...
select d.nombre_depto as departamento, count(*)
...
/*
SELECT e.*, AVG(sal_emp) OVER () as promedio_empresa, d.nombre_depto
FROM empleados e
JOIN departamentos d ON e.id_depto = d.id_depto
WHERE e.sal_emp >= (SELECT AVG(sal_emp) FROM empleados)
ORDER BY d.nombre_depto;
